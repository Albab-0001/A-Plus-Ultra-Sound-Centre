...
const ServiceCard = ({ icon, title, description, scanType }: ServiceCardProps) => {
  ...
  return (
      ...
      <i className={`${icon} text-4xl text-blue-500 mb-6 block`}></i>
      <h3 className="text-2xl font-semibold mb-4 text-gray-800">{title}</h3>
      <p className="text-lg text-black leading-relaxed flex-grow mb-6">{description}</p>
      <div className="flex justify-center mt-auto">
        <Link
          to={bookingUrl}
          className="inline-block bg-green-500 hover:bg-green-600 text-white px-5 py-1.5 rounded-lg text-center text-lg font-semibold transition-all duration-300 hover:shadow-lg hover:transform hover:-translate-y-1 w-auto max-w-[60%]"
        >
          Book Slot
        </Link>
      </div>
